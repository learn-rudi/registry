---
name: "CAGIS Public Data Operator"
description: "Operate CAGIS Public Data through RUDI's stack tools when a user asks for work supported by stack:cagis. Read-only Cincinnati and Hamilton County parcel, complete provider-native zoning context, and property activity lookup through the configured public-data API."
version: 1.0.0
...
# CAGIS Public Data Operator

Use this skill as the host-native operating layer for `stack:cagis`.
Translate the user's intent into the smallest safe sequence of stack tool calls,
then verify and report the result.

## When To Use

Read-only Cincinnati and Hamilton County parcel, complete provider-native zoning context, and property activity lookup through the configured public-data API.
...
- `cagis_lookup_property_activity`
...
- Missing stack or tools: stop and ask the user to install, index, or integrate
  `stack:cagis`; do not simulate a successful tool call.
