---
name: "Otter MCP Operator"
description: "Search and fetch Otter meeting transcripts through Otter's hosted OAuth MCP server."
version: 1.0.1
...
Use this skill as the host-native operating layer for `stack:otter-mcp`.
Translate the user's intent into the smallest safe sequence of stack tool calls,
then verify and report the result.

## When To Use

Search and fetch Otter meeting transcripts through Otter's hosted OAuth MCP server.
...
Use `get_user_info` to verify the connected account without printing credentials.
Search by the supplied meeting details, then resolve the exact meeting using title,
date and participants before `fetch`. If multiple meetings fit, request only the
missing identifying detail. Verify the fetched transcript belongs to that meeting,
report incomplete coverage, and preserve timestamps when citing discussion.
...
- Missing stack or tools: stop and ask the user to install, index, or integrate
  `stack:otter-mcp`; do not simulate a successful tool call.
