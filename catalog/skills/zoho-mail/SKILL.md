---
name: "Zoho Mail Operator"
description: "Zoho Mail tools for OAuth accounts, folder and message search, email send, draft, reply, deletion, threads, and attachments"
version: 1.0.1
...
Use this skill as the host-native operating layer for `stack:zoho-mail`.
Translate the user's intent into the smallest safe sequence of stack tool calls,
then verify and report the result.

## When To Use

Zoho Mail tools for OAuth accounts, folder and message search, email send, draft, reply, deletion, threads, and attachments
...
Use `zoho_account_list`/`zoho_account_current` to resolve the intended account.
Choose search/read, draft, reply, send or deletion from the requested operation.
For replies, inspect the exact thread and recipient list; for attachments, resolve
the returned message and attachment IDs. Prepare draft/body/recipient changes before
an authorized write. Inspect the resulting draft, thread or sent message when
supported and report its ID. Check account and thread history before retrying an
uncertain send; changing accounts is not implied by a message search.
...
- Missing stack or tools: stop and ask the user to install, index, or integrate
  `stack:zoho-mail`; do not simulate a successful tool call.
