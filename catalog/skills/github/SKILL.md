...
description: "Manage GitHub repositories, branches, contents, collaborators, issues, pull requests, releases, Actions, artifacts, and direct REST API calls"
version: 1.0.1
category: "code"
tags:
  - rudi
...
Resolve the exact owner/repository before operating. For repository content, inspect
the target branch/ref and file; for an issue, read its number and current state;
for a pull request, inspect base/head, diff and checks before proposing a change.
Use the narrow named repository, issue or PR tools first. Use the generic REST tool
only after verifying its method, endpoint and payload in the live schema.
Prepare requested mutations against those exact targets, apply only the authorized
action, and read back the resulting object or revision. A request to inspect a PR
does not authorize merging it or changing repository visibility.
