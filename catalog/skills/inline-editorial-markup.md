...
version: 1.0.0
category: writing
tags: [writing, editing, markup, review, redline, html, transcripts]
requires:
  stacks:
...
Use a RUDI stack as a supporting capability when it is available and useful:

- For first-class editorial artifacts, prefer `stack:editorial-markup` once installed.
- For browser/render/export QA, discover and prefer `stack:web-export` or equivalent Playwright-backed tooling when the editorial stack is unavailable.
- For transcript/media extraction, use the relevant extraction or audio stack before editorial markup when the source is media or a URL.
- Do not require a stack just to generate a self-contained HTML file; plain filesystem generation is acceptable as a fallback.
- When a stack is unavailable, fall back to local Playwright/browser tooling or explicit DOM/layout checks and report the gap.

In practice: invoking `$inline-editorial-markup` should make the agent aware that rendering/export QA stacks can help, then use them when the task calls for visual verification.
