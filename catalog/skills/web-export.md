---
name: "Web Export Operator"
description: "Operate Web Export through RUDI's stack tools when a user asks for work supported by stack:web-export. Export HTML to PDF/PNG with mode detection, artifact verification, and structured diagnostics."
version: 1.0.0
...
Use this skill as the host-native operating layer for `stack:web-export`.
Translate the user's intent into the smallest safe sequence of stack tool calls,
then verify and report the result.

## When To Use

Export HTML to PDF/PNG with mode detection, artifact verification, and structured diagnostics.
...
- Missing stack or tools: stop and ask the user to install, index, or integrate
  `stack:web-export`; do not simulate a successful tool call.
