...
description: "Operate PostgreSQL through RUDI's stack tools when a user asks for work supported by stack:postgres. Query and manage PostgreSQL databases. Works with Neon, Railway, Supabase, and any PostgreSQL connection."
version: 1.0.0
category: "data"
tags:
  - rudi
...
Use this skill as the host-native operating layer for `stack:postgres`.
Translate the user's intent into the smallest safe sequence of stack tool calls,
then verify and report the result.

## When To Use

Query and manage PostgreSQL databases. Works with Neon, Railway, Supabase, and any PostgreSQL connection.
...
- Missing stack or tools: stop and ask the user to install, index, or integrate
  `stack:postgres`; do not simulate a successful tool call.
