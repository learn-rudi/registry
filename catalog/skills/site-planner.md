---
name: "Site Planner Operator"
description: "Operate Site Planner through RUDI's stack tools when a user asks for work supported by stack:site-planner. Fixed-root MCP adapter for inspecting, generating, optimizing, previewing, forking, and applying changes to a pinned local Site Planner workspace."
version: 1.0.0
...
# Site Planner Operator

Use this skill as the host-native operating layer for `stack:site-planner`.
Translate the user's intent into the smallest safe sequence of stack tool calls,
then verify and report the result.

## When To Use

Fixed-root MCP adapter for inspecting, generating, optimizing, previewing, forking, and applying changes to a pinned local Site Planner workspace.
...
- Missing stack or tools: stop and ask the user to install, index, or integrate
  `stack:site-planner`; do not simulate a successful tool call.
