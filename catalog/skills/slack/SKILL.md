---
name: "Slack Operator"
description: "Send messages, search channels, upload files, and manage reactions in Slack"
version: 1.0.1
...
Use this skill as the host-native operating layer for `stack:slack`.
Translate the user's intent into the smallest safe sequence of stack tool calls,
then verify and report the result.

## When To Use

Send messages, search channels, upload files, and manage reactions in Slack
...
For discovery, use `slack_search` or read the resolved channel/thread. Resolve the
exact workspace, channel ID and thread before preparing communication; do not infer
a destination from a similarly named channel. Sending, updating, reacting and
uploading each require scope that covers that action. After an authorized write,
record the returned message timestamp/ID and read back the exact channel/thread
when supported. Never retry an uncertain send without checking for the message.
...
- Missing stack or tools: stop and ask the user to install, index, or integrate
  `stack:slack`; do not simulate a successful tool call.
