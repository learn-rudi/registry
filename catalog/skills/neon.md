---
name: "Neon Operator"
description: "Operate Neon through RUDI's stack tools when a user asks for work supported by stack:neon. Guarded Neon platform operations for projects, branches, connection URI retrieval, and MCP config generation."
version: 1.0.0
category: "data"
tags:
  - rudi
...
Use this skill as the host-native operating layer for `stack:neon`.
Translate the user's intent into the smallest safe sequence of stack tool calls,
then verify and report the result.

## When To Use

Guarded Neon platform operations for projects, branches, connection URI retrieval, and MCP config generation.
...
- Missing stack or tools: stop and ask the user to install, index, or integrate
  `stack:neon`; do not simulate a successful tool call.
