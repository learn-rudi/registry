---
name: "Document QA Operator"
description: "Visual QA inspection for HTML documents using screenshots and Claude Vision."
version: 1.0.1
category: "documents"
tags:
  - rudi
...
# Document QA Operator

Use this skill as the host-native operating layer for `stack:document-qa`.
Translate the user's intent into the smallest safe sequence of stack tool calls,
then verify and report the result.

## When To Use

Visual QA inspection for HTML documents using screenshots and Claude Vision.
...
Pass the exact accepted HTML artifact to `document_qa_inspect` with the intended
page/viewport settings supported by the live schema. Inspect screenshots for
clipping, overflow, overlap, missing content, pagination and unreadable text.
Tie each finding to a visible page or region and verify repairs with a new inspection.
This stack checks visual output; web-export renders deliverable PDF/PNG artifacts.
A successful render alone does not establish visual correctness.
...
- Missing stack or tools: stop and ask the user to install, index, or integrate
  `stack:document-qa`; do not simulate a successful tool call.
