---
name: "Twilio SMS Operator"
description: "Send SMS messages and inspect recent Twilio messages through a guarded MCP interface."
version: 1.0.1
...
Use this skill as the host-native operating layer for `stack:twilio-sms`.
Translate the user's intent into the smallest safe sequence of stack tool calls,
then verify and report the result.

## When To Use

Send SMS messages and inspect recent Twilio messages through a guarded MCP interface.
...
Use `twilio_config_status` to verify readiness without exposing credentials. Resolve
the exact recipient, sender and final message body; a search/read request does not
authorize sending. After an authorized `twilio_send_sms`, retain the returned SID
and use `twilio_get_message` to inspect status. Distinguish accepted/queued from
delivered. Check message history before retrying an uncertain send to avoid duplicates.
...
- Missing stack or tools: stop and ask the user to install, index, or integrate
  `stack:twilio-sms`; do not simulate a successful tool call.
