---
name: "MS Office Operator"
description: "Operate MS Office through RUDI's stack tools when a user asks for work supported by stack:ms-office. Read and extract content from Microsoft Word (.docx) and Excel (.xlsx) documents"
version: 1.0.0
...
Use this skill as the host-native operating layer for `stack:ms-office`.
Translate the user's intent into the smallest safe sequence of stack tool calls,
then verify and report the result.

## When To Use

Read and extract content from Microsoft Word (.docx) and Excel (.xlsx) documents
...
- Missing stack or tools: stop and ask the user to install, index, or integrate
  `stack:ms-office`; do not simulate a successful tool call.
