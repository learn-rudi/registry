---
name: "Local Whisper Operator"
description: "Operate Local Whisper through RUDI's stack tools when a user asks for work supported by stack:whisper. Transcribe audio locally with faster-whisper. No API key needed - runs on your machine."
version: 1.0.0
category: "ai-local"
tags:
  - rudi
...
# Local Whisper Operator

Use this skill as the host-native operating layer for `stack:whisper`.
Translate the user's intent into the smallest safe sequence of stack tool calls,
then verify and report the result.

## When To Use

Transcribe audio locally with faster-whisper. No API key needed - runs on your machine.
...
- Missing stack or tools: stop and ask the user to install, index, or integrate
  `stack:whisper`; do not simulate a successful tool call.
