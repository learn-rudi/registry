...
description: "Operate Stripe through RUDI's stack tools when a user asks for work supported by stack:stripe. Create and inspect Stripe products, one-time prices, and Payment Links through guarded RUDI MCP tools"
version: 1.0.0
category: "payments"
tags:
  - rudi
...
Use this skill as the host-native operating layer for `stack:stripe`.
Translate the user's intent into the smallest safe sequence of stack tool calls,
then verify and report the result.

## When To Use

Create and inspect Stripe products, one-time prices, and Payment Links through guarded RUDI MCP tools
...
- `stripe_create_one_time_payment_link`
...
- Missing stack or tools: stop and ask the user to install, index, or integrate
  `stack:stripe`; do not simulate a successful tool call.
