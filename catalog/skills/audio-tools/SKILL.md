...
description: "Transcribe, enrich, and query audio or video-linked speech. Accepts local files, direct media URLs, supported video-page URLs, or base64 input."
version: 1.0.1
category: "media"
tags:
  - rudi
...
Resolve whether the input is a local file, direct media URL, supported video page,
or base64 payload using the live schema. Use `audio_transcribe` for speech text,
`audio_enrich` for analysis of an existing transcript, and `audio_query` for questions
against indexed material. Reuse returned transcript IDs. Verify source identity,
language, transcript length and any missing segments before synthesis. Local
Whisper is a separate transcription route; provider-only transcription does not
by itself provide this stack’s enrichment and query workflow.
