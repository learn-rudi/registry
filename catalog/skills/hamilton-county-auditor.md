---
name: "Hamilton County Auditor Public Data Operator"
description: "Operate Hamilton County Auditor Public Data through RUDI's stack tools when a user asks for work supported by stack:hamilton-county-auditor. Read-only authoritative parcel ownership, assessed value, characteristics, and sales facts through the configured public-data API."
version: 1.0.0
...
requires:
  stacks:
    - stack:hamilton-county-auditor
---

# Hamilton County Auditor Public Data Operator

Use this skill as the host-native operating layer for `stack:hamilton-county-auditor`.
Translate the user's intent into the smallest safe sequence of stack tool calls,
then verify and report the result.

## When To Use

Read-only authoritative parcel ownership, assessed value, characteristics, and sales facts through the configured public-data API.
...
- Missing stack or tools: stop and ask the user to install, index, or integrate
  `stack:hamilton-county-auditor`; do not simulate a successful tool call.
