---
name: "Municode Public Codes Operator"
description: "Operate Municode Public Codes through RUDI's stack tools when a user asks for work supported by stack:municode. Read-only, fixed-job reviewed baseline zoning-code evidence bundles for Cincinnati from release-configured mappings and complete CAGIS zoning context."
version: 1.0.0
...
# Municode Public Codes Operator

Use this skill as the host-native operating layer for `stack:municode`.
Translate the user's intent into the smallest safe sequence of stack tool calls,
then verify and report the result.

## When To Use

Read-only, fixed-job reviewed baseline zoning-code evidence bundles for Cincinnati from release-configured mappings and complete CAGIS zoning context.
...
- Missing stack or tools: stop and ask the user to install, index, or integrate
  `stack:municode`; do not simulate a successful tool call.
