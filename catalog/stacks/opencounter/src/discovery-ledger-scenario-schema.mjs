...
import { createScenarioBranchJobSha256, createScenarioBranchLedgerSha256 } from "./discovery-scenario-wave.mjs";
...
export const JOB_ID_PATTERN = /^ocdj_[0-9a-f]{64}$/;

export const LEDGER_ID_PATTERN = /^ocdl_[0-9a-f]{64}$/;
...
export function validateScenarioBranchCampaignLedger(ledger) {
  exactKeys(ledger, [
...
export function validateCatalogCampaignIdentity(value) {
...
export function validateCatalogCampaignJobFields(job, index, ledger) {
  exactKeys(job, [
...
export function validateZoningPortfolioLocationFixture(value, jobIndex) {
...
export function validateScenario(value, jobIndex) {
...
export function validateLease(job, index) {
...
export function validateNextAction(job, index, catalogId) {
  if (job.nextAction === null) return;
  const action = record(job.nextAction, `ledger.jobs[${index}].nextAction`);
  if (action.kind === "start") {
    exactKeys(action, ["input", "kind"], "nextAction");
