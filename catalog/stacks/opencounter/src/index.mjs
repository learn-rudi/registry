...
const tools = [
  ...
  { name: "opencounter_start_zoning_guidance", description: "Start one Cincinnati OpenCounter Zoning run from an exact admitted catalog entry after a read-only provider fingerprint preflight.", required: ["address", "catalogEntryId", "catalogId", "jurisdiction", "schemaVersion"], properties: { address: { type: "string", minLength: 1, maxLength: 500 }, catalogEntryId: { type: "string", pattern: "^[a-z0-9_]+(?:\\.[a-z0-9_]+)+$", maxLength: 200 }, catalogId: { const: "cincinnati-opencounter-zoning-use-catalog-v1" }, jurisdiction: { const: "cincinnati-oh" }, schemaVersion: { const: 1 } } },
...
const server = new Server({ name: "rudi-opencounter", version: "0.3.0" }, { capabilities: { tools: {} } });
